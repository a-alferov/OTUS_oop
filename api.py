...
class ClientsInterestsRequest:
    ...
    def get_response(self, **kwargs) -> tuple[dict, int]:
        ...
        return {str(k): get_interests() for k in self.client_ids}, OK
...
def method_handler(request, ctx, _) -> tuple[dict, int]:
    ...
    try:
        response, code = methods[mr.method](**mr.arguments).get_response(request=mr, context=ctx)
